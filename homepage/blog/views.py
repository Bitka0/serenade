...
def show(request, url):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			comment = Comment(blogentry = entry)
	...
	context = util.generateContext(request, contextType = 'RequestContext', title = entry.title, entry = entry, groups = entry.group.all(), tags = entry.tag.all(), commentform = form, comments = Comment.objects.filter(blogentry = entry))
